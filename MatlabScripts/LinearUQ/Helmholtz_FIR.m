function [fref,gref] = Helmholtz_FIR(solver,FIR_coef,MagRs,ArgRs,MagRn,ArgRn,alpha_damp,configs,s_init)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Helmholtz solver
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Inputs:
%       solver:              Iteration algorithm ('Fixpoint' or 'Secant')
%       FIR_coef:         FIR model coefficients         
    ...
    switch solver
        case 'Fixpoint'
            ...
            while abs(ds) > 1e-4 && iter<iterlim
                ...
                delta_t = 4e-4; nb = size(FIR_coef,2);
                complex_vector = zeros(nb,1);
                for index_i = 1:nb
                    complex_vector(index_i) = exp(-(index_i-1)*delta_t*s);
                end
                
                flame.FTF = FIR_coef*complex_vector;
        ...
        case 'Secant'
            ...
            while 1 %abs(ds) > 1e-2 && iter < iterlim
                ...
                %%%%%%% Construct Flame Model %%%%%%%%%
                delta_t = 4e-4; nb = size(FIR_coef,2);
                complex_vector1 = zeros(nb,1);
                complex_vector2 = zeros(nb,1);
                
                for index_i = 1:nb
                    complex_vector1(index_i) = exp(-(index_i-1)*delta_t*svec(1));
                    complex_vector2(index_i) = exp(-(index_i-1)*delta_t*svec(2));
                end
                
                flame.FTF = FIR_coef*complex_vector1;
                H1 = sflame(mesh, index, dim, l, flame, thin);
                flame.FTF = FIR_coef*complex_vector2;
